...
public class Util {
	...
	/**
	 * Return the max alert ordered by maxBy under username.
	 */
	public static Entity getAlertMax(String kind, String username, String maxBy) {
		logger.log(Level.INFO, "Search the max entity based on search criteria");
		Query q = new Query(kind);
		q.addFilter("username", FilterOperator.EQUAL, username);
		q.addSort(Entity.KEY_RESERVED_PROPERTY, SortDirection.DESCENDING);
		q.addSort(maxBy, SortDirection.DESCENDING);
		PreparedQuery pq = datastore.prepare(q);

		List<Entity> maxList = pq.asList(FetchOptions.Builder.withLimit(1));
		if (maxList.size() == 1) {
	...
	/**
	 * Return the max deal ordered by maxBy under alertKey.
	 */
	public static Entity getDealMax(Key ancestor, String kind, String maxBy) {
		logger.log(Level.INFO, "Search the max entity based on search criteria");
		Query q = new Query(kind);
	...
	public static Iterable<Entity> listEntities(String kind, String searchBy,
			Object searchFor) {
		logger.log(Level.INFO, "Search entities based on search criteria:" + searchBy + ", " + searchFor);
		Query q = new Query(kind);
	 ...
	 */
	public static Iterable<Entity> listEntities(String kind, String searchBy,
			String searchFor, String sortBy, SortDirection direction) {
		logger.log(Level.INFO, "Search entities based on search criteria:" + searchBy + ", " + searchFor);
		Query q = new Query(kind);
		if (searchFor != null && !"".equals(searchFor)) {
			q.addFilter(searchBy, FilterOperator.EQUAL, searchFor);
			q.addSort(Entity.KEY_RESERVED_PROPERTY, SortDirection.DESCENDING);
			q.addSort(sortBy, direction);
		}
		PreparedQuery pq = datastore.prepare(q);
		return pq.asList(FetchOptions.Builder.withLimit(100));
	}

	public static Iterable<Entity> listEntitiesSingleSort(String kind, String searchBy,
			String searchFor, String sortBy, SortDirection direction) {
		logger.log(Level.INFO, "Search entities based on search criteria:" + searchBy + ", " + searchFor);
		Query q = new Query(kind);
		if (searchFor != null && !"".equals(searchFor)) {
...
//			q.addSort(Entity.KEY_RESERVED_PROPERTY, SortDirection.DESCENDING);
	...
	public static List<Entity> listChildrenAsList(String kind, Key ancestor) {
		logger.log(Level.INFO, "Search entities based on parent");
		Query q = new Query(kind);
		q.setAncestor(ancestor);
		q.addFilter(Entity.KEY_RESERVED_PROPERTY, FilterOperator.GREATER_THAN, ancestor);
...
//	public static Iterable<Entity> listChildren(String kind, Key ancestor, String sortBy, SortDirection direction) {
//		logger.log(Level.INFO, "Search entities based on parent");
//		Query q = new Query(kind);
//		q.setAncestor(ancestor);
//		q.addFilter(Entity.KEY_RESERVED_PROPERTY, FilterOperator.GREATER_THAN, ancestor);
...
//	}
	...
	public static String getTodayMDYSlash() {
		DateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy");
		Calendar today = Calendar.getInstance();
		return dateFormat.format(today.getTime());
	}
	
	public static String getTodayMDY() {
		DateFormat dateFormat = new SimpleDateFormat("MMddyyyy");
		Calendar today = Calendar.getInstance();
		return dateFormat.format(today.getTime());
	}

	public static String getTodayMY() {
