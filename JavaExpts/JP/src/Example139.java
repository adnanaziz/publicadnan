...
class Nfa {
  ...
  public Nfa(Integer startState, Integer exitState) {
    ...
    if (!startState.equals(exitState)) trans.put(exitState,
  ...
  public void addTrans(Integer s1, String lab, Integer s2) {
    ...
    if (trans.containsKey(s1)) s1Trans = trans.get(s1);
    else {
        ...
        for (Integer s : S) {
          ...
          for (Transition tr : trans.get(s)) {
            if (tr.lab != null) { // Already a transition on lab
              Set<Integer> toState;
              if (STrans.containsKey(tr.lab)) toState = STrans.get(tr.lab);
  ...
  static Map<Integer, Map<String, Integer>>
    ...
    for (Map.Entry<Set<Integer>, Map<String, Set<Integer>>> entry : trans
        .entrySet()) {
  ...
  static Set<Integer> acceptStates(Set<Set<Integer>> states,
    ...
    for (Set<Integer> state : states)
      if (state.contains(exit)) acceptStates.add(renamer.get(state));
    return acceptStates;
  }

  public Dfa toDfa() {
    Map<Set<Integer>, Map<String, Set<Integer>>> cDfaTrans = compositeDfaTrans(
        startState, trans);
    Set<Integer> cDfaStart = epsilonClose(Collections.singleton(startState),
        trans);
    Set<Set<Integer>> cDfaStates = cDfaTrans.keySet();
    Map<Set<Integer>, Integer> renamer = mkRenamer(cDfaStates);
    Map<Integer, Map<String, Integer>> simpleDfaTrans = rename(renamer,
        cDfaTrans);
    Integer simpleDfaStart = renamer.get(cDfaStart);
    Set<Integer> simpleDfaAccept = acceptStates(cDfaStates, renamer,
        exitState);
...
class Dfa {
  ...
  public void writeDot(String filename) throws IOException {
    ...
    for (Integer state : trans.keySet())
      if (acceptStates.contains(state)) out.println("n" + state
          + " [peripheries=2];");
...
class Seq extends Regex {
  ...
  public Nfa mkNfa(Nfa.NameSource names) {
    ...
    for (Map.Entry<Integer, List<Nfa.Transition>> entry : nfa1.getTrans()
        .entrySet())
      nfa0.addTrans(entry);
    for (Map.Entry<Integer, List<Nfa.Transition>> entry : nfa2.getTrans()
        .entrySet())
      nfa0.addTrans(entry);
