...
class Example88 {
  public static void main(String[] args) {
    if (args.length != 3) System.out.println("Usage: Example88 yyyy mm dd\n");
    else {
      try {
        MyDate d = new MyDate(Integer.parseInt(args[0]),
            Month.toMonth(Integer.parseInt(args[1])),
            Integer.parseInt(args[2]));
        System.out.println(d + " is " + d.weekday() + " in week "
            + d.weekNumber());
      } catch (Exception e) {
...
class MyDate {
  ...
  public MyDate(int yy, Month mm, int dd) throws Exception {
    if (ok(yy, mm, dd)) {
      ...
      this.mm = mm;
      this.dd = dd;
    } else throw new Exception("Illegal date (" + yy + "," + mm + "," + dd
        + ")");
  }
  ...
  public static int weekNumber(int y, Month m, int d) {
    ...
    if (week == 0) return (yday + yearDays(y - 1) - wday + 10) / 7;
    else return week;
  }
