...
class Example122 {
  ...
  public static void main(String[] args) {
    if (args.length != 2) System.out
        .println("Usage: java Example122 <iterations> <word>");
    else {
      final int count = Integer.parseInt(args[0]);
      final String id = args[1];
      for (int i = 0; i < keywordarray.length; i++)
        if (isKeyword1(keywordarray[i]) != isKeyword2(keywordarray[i])) System.out
            .println("Error at i = " + i);
      if (isKeyword1(id) != isKeyword2(id)) System.out
          .println("Error at id = " + id);
