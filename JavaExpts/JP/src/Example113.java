// Example 113 from page 85 of Java Precisely second edition (The MIT Press 2005)
...
class MyLinkedList<T> implements MyList<T> {
  ...
  public boolean equals(MyList<T> that) {
    if (that != null && this.size == that.getCount()) {
      ...
      while (thisnode != null) {
        if (!thatiter.hasNext()) throw new RuntimeException(
            "Impossible: MyLinkedList<T>.equals");
        // assert next() will succeed (because of the above size test)
        if (!thisnode.item.equals(thatiter.next())) return false;
...
class Example113 {
  public static void main(String[] args) {
    MyLinkedList<Double> dLst = new MyLinkedList<Double>(7.0, 9.0, 13.0, 0.0);
    for (double d : dLst)
      System.out.print(d + " ");
    System.out.println();
    MyList<Integer> iLst =
